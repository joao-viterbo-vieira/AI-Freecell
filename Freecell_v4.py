...
class FreeCellGame:
    ...
    def _is_valid_sequence(self, cards):
        ...
        for i in range(len(cards) - 1):
            if not (cards[i].rank + 1 == cards[i + 1].rank and cards[i].color != cards[i + 1].color):
                return False
        return True
    ...
    def draw(self, highlight_move=None, stats=None, algorithm="A*", hint_move=None):
        ...
        for i, cascade in enumerate(self.cascades):
            ...
            for j, card in enumerate(cascade):
                card_y = y + j * 30
                is_regular_highlight = (selected_card and selected_source == ("cascade", i) and j == len(cascade) - 1) or \
