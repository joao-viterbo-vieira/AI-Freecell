...
auto_moves_enabled = True  # New global variable to control automoves
...
class FreeCellGame:
    ...
    def make_move(self, move, is_player_move=False):
        ...
        if is_player_move:
            self.player_moves.append(("manual", move))
        else:
            self.moves.append(move)

    def auto_move_to_foundations(self):
        """Execute safe automoves to foundations after a player move, if enabled and after first move."""
        ...
        while changed:
            ...
            for i, cascade in enumerate(self.cascades):
                if cascade:
                    ...
                    if self.can_move_to_foundation(card):
                        foundation = self.foundations[card.suit]
                        rank_needed = foundation[-1].rank + 1 if foundation else 1
                        if card.rank == rank_needed:
                            safe = True
                            for j, other_cascade in enumerate(self.cascades):
                                if i != j and other_cascade and other_cascade[-1].rank < rank_needed:
                                    safe = False
                                    break
                            for free_card in self.free_cells:
                                if free_card and free_card.rank < rank_needed:
                                    safe = False
                                    break
                            if safe:
                    ...
                    if card and self.can_move_to_foundation(card):
                        ...
                        if card.rank == rank_needed:
                            ...
                            for cascade in self.cascades:
                                if cascade and cascade[-1].rank < rank_needed:
                                    safe = False
                                    break
                            for j, free_card in enumerate(self.free_cells):
                                if i != j and free_card and free_card.rank < rank_needed:
                                    safe = False
                                    break
                            if safe:
    ...
    def undo_last_move(self):
        """Undo the last player move, including any subsequent automoves."""
        if not self.player_moves:
            return False
        last_action = self.player_moves.pop()
        action_type, move = last_action
        ...
        # Reverse any automoves that followed this manual move
        while self.player_moves and self.player_moves[-1][0] == "auto":
            _, auto_move = self.player_moves.pop()
            auto_move_type, auto_source_type, auto_source_idx, auto_dest = auto_move
            card = self.foundations[auto_dest].pop()
            if auto_source_type == "cascade":
                self.cascades[auto_source_idx].append(card)
            else:  # free_cell
                self.free_cells[auto_source_idx] = card
                ...
                for i in range(8):
                    ...
                    if cascade_x <= x <= cascade_x + CARD_WIDTH and 250 <= y:
                        ...
                        if src_idx != i and (not self.cascades[i] or (selected_sequence[0].rank == self.cascades[i][-1].rank - 1 and selected_sequence[0].color != self.cascades[i][-1].color)):
                            ...
                            self.auto_move_to_foundations()  # Check for automoves after manual move
                            move_made = True
...
def main():
    ...
    auto_moves_enabled = True  # Start with automoves enabled
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                ...
                if search_box_rect.collidepoint((x, y)):
                    ...
                    if 0 <= y <= 60:
                        ...
                        stats = hint_move = last_moved_card = None
                        selected_sequence = selected_sequence_source = None
                        # No automove check here at start of single-player mode
